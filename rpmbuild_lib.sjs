...
function dispatchEncodeType(toPath, glob, type, encodeRPath , param){/*{{{*/
    ...
    for (pro in fileList) {
        ...
        switch(type){
            case "closure":
                execSync("java -jar /home/program/lib/closurecompiler.jar --js  \""+fileList[pro]+"\" --js_output_file "+toPath+"/"+fileName);
                break;
            case "css_sprite":
                var css = new _CSS(); 
            ...
            case "css":
                //yui compress
                execSync("cat "+fileList[pro]+" | java -jar  /home/program/lib/yuicompressor-2.4.6.jar --charset utf8 --type css -o "+toPath+"/"+fileName); 
                break;
            case "php_c1"://only remove comment
                //yui compress
                var res = execSync("php /home/program/lib/phpcompress/phpcompress.php "+ fileList[pro] +" "+toPath+"/"+fileName+" c1"); 
                print_r(res);
                break;
            case "xml_encoding":
